class Camera
{
  Camera()
  {
    ...
    farPlane         = 10000;
     ...
     Camera(float x,float y,float z,float passtheta,float passphi,float passmove,float passturn)
  {
    position      = new PVector( x, y, z ); // initial position
    theta         = passtheta; // rotation around Y axis. Starts with forward direction as ( 0, 0, -1 )
    phi           = passphi; // rotation around X axis. Starts with up direction as ( 0, 1, 0 )
    moveSpeed     = passmove;
    turnSpeed     = passturn; // radians/sec
  ...
  void HandleKeyReleased()
  {
    ...
    //Added to keep good recording angles
    if (key == 'f') System.out.print(position + "\n" + theta + "\n" + phi);
